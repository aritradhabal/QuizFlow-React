...
function Sideitems() {
  ...
  return (
    <>
    <div className="flex flex-row h-10 mb-2 mx-2 rounded-md border-0 justify-evenly items-center bg-transparent md:mx-0 md:mb-0 md:border-0 md:bg-white md:w-auto md:h-auto md:flex md:flex-col md:justify-center md:items-stretch md:gap-y-2 md:mt-10 md:px-5 ">
            <button onClick={create} 
                className={`Create cursor-pointer flex items-center justify-start transition duration-350 gap-x-2 rounded-sm hover:bg-[#E2E8F0] py-1 px-2 
                    ${SideBar === 'Create' ? 'bg-[#FDEDF7] hover:bg-[#FDEDF7]' : ''}`}>
                    {SideBar === 'Create' ? <AiFillThunderbolt className='fill-pink-500 transition-all duration-350 transform scale-110'/> 
                    :<AiFillThunderbolt className='fill-gray-700 transition-all duration-300 transform scale-100 '/>}
